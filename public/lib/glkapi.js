...
function update() {
    ...
    const current_page_pg = stylehints.buffer['.Style_normal']?.['background-color']
    ...
    if (Array.isArray(stylehints)) {
        ...
        for (const [stylenum, stylehint] of stylehints.entries()) {
            for (const [css_style, value] of Object.entries(stylehint)) {
                const selector = `.Style_${STYLE_NAMES[stylenum]}${['margin-left', 'text-indent','text-align'].includes(css_style) ? '_par' : ''}`
                if (!res[selector]) {
                    res[selector] = {}
                }
...
function add_css_style(styles, style, hint, value) {
    const selector = `.Style_${STYLE_NAMES[style]}${hint <= Const.stylehint_Justification ? '_par' : ''}`
    var stylevalue
